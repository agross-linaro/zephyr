...
struct esp8266_data {
	...
	struct socket_data *active_sock;
...
{
	foo_data.last_error = 0;
	if (!foo_data.active_sock) {
		k_sem_give(&foo_data.response_sem);
	} else {
		k_sem_give(&foo_data.active_sock->sock_send_sem);
	}
	foo_data.active_sock = NULL;
}
...
static int esp8266_put(struct net_context *context)
{
	...
	if (foo_data.socket_data[id].connected) {
		...
		if (send_at_cmd(&foo_data.socket_data[id], msg, MDM_CMD_TIMEOUT) < 0) {
...
static int esp8266_process_setup_read(struct net_buf **buf)
{
	...
	*buf = net_buf_skip(*buf, hdr_len);
	...
	SYS_LOG_INF("MATCH +IPD (len:%u)", foo_data.data_len);
	return 0;
}
